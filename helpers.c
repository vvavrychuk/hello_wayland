...
struct wl_shm_pool *hello_create_memory_pool(int file)
{
	...
	if (fstat(file, &stat) != 0)
        ...
        data->memory = mmap(0, data->capacity*sizeof(pixel), PROT_READ,
			    MAP_SHARED, data->fd, 0);

        if (data->memory == MAP_FAILED)
                goto cleanup_alloc;

        pool = wl_shm_create_pool(shm, data->fd, data->capacity*sizeof(pixel));
...
struct wl_buffer *hello_create_buffer(struct wl_shm_pool *pool, unsigned width, unsigned height)
{
	...
	pool_data = wl_shm_pool_get_user_data(pool);
        buffer = wl_shm_pool_create_buffer(pool, pool_data->size*sizeof(pixel), width,
					   height, width*sizeof(pixel), PIXEL_FORMAT_ID);
...
)
{
	...
	wl_pointer_set_cursor(wl_pointer, serial, pointer_data->surface,
			      pointer_data->hot_spot_x, pointer_data->hot_spot_y);
}

static void pointer_leave
(
	void *data, struct wl_pointer *wl_pointer, uint32_t serial, struct wl_surface *wl_surface
)
{
}
